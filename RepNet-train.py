...
from RepNet import VehicleID_All, VehicleID_MC, FocalLoss, RepNet
...
def train(resume,
  ...
  # 数据集
  train_set = VehicleID_All(root=data_root, transforms=None, mode='train')
  test_set = VehicleID_All(root=data_root, transforms=None, mode='test')
  train_loader = torch.utils.data.DataLoader(dataset=train_set,
                                             batch_size=16,
                                             shuffle=True,
  ...
  for epoch_i in range(epoch):
    ...
    for batch_i, (data, label) in enumerate(train_loader):  # 遍历每一个batch
      ...
      # ------------- calculate loss
      # branch1 loss
      loss_m = loss_func_1(output_1[:, :model_attribs],
                           label[:, 0])  # vehicle model
      loss_c = loss_func_1(output_1[:, model_attribs:],
                           label[:, 1])  # vehicle color
    ...
    if test_acc > best_acc:
      ...
      torch.save(net.state_dict(), os.path.join(model_save_dir,
                                                model_save_name))
      print('<= {} saved.'.format(model_save_name))
