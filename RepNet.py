    ...
    with open(txt_f_path, 'r', encoding='utf-8') as f_h:
      for line in f_h.readlines():
        ...
        if os.path.isfile(img_path):
          ...
          label = np.array([int(line[1]), int(line[2]), int(tr_id)], dtype=int)
          self.lables.append(torch.Tensor(label))

    assert len(self.imgs_path) == len(self.lables)
    print('=> total %d samples loaded in %s mode' %
          (len(self.imgs_path), mode))
...
class RepNet(torch.nn.Module):
  def __init__(self, out_ids, out_attribs):
    ...
    self.conv1 = torch.nn.Sequential(self.conv1_1, self.conv1_2, self.conv1_3,
                                     self.conv1_4, self.conv1_5)
    ...
    self.conv2 = torch.nn.Sequential(self.conv2_1, self.conv2_2, self.conv2_3,
                                     self.conv2_4, self.conv2_5)
    ...
    self.conv3 = torch.nn.Sequential(self.conv3_1, self.conv3_2, self.conv3_3,
                                     self.conv3_4, self.conv3_5, self.conv3_6,
                                     self.conv3_7)
    ...
    self.conv4_1 = torch.nn.Sequential(self.conv4_1_1, self.conv4_1_2,
                                       self.conv4_1_3, self.conv4_1_4,
                                       self.conv4_1_5, self.conv4_1_6,
                                       self.conv4_1_7)
    ...
    self.conv4_2 = torch.nn.Sequential(self.conv4_2_1, self.conv4_2_2,
                                       self.conv4_2_3, self.conv4_2_4,
                                       self.conv4_2_5, self.conv4_2_6,
                                       self.conv4_2_7)
    ...
    self.conv5_1 = torch.nn.Sequential(self.conv5_1_1, self.conv5_1_2,
                                       self.conv5_1_3, self.conv5_1_4,
                                       self.conv5_1_5, self.conv5_1_6,
                                       self.conv5_1_7)
    ...
    self.conv5_2 = torch.nn.Sequential(self.conv5_2_1, self.conv5_2_2,
                                       self.conv5_2_3, self.conv5_2_4,
                                       self.conv5_2_5, self.conv5_2_6,
                                       self.conv5_2_7)
    ...
    self.FC6_1 = torch.nn.Sequential(self.FC6_1_1, self.FC6_1_2, self.FC6_1_3,
                                     self.FC6_1_4, self.FC6_1_5, self.FC6_1_6)
    ...
    self.FC6_2 = torch.nn.Sequential(self.FC6_2_1, self.FC6_2_2, self.FC6_2_3,
                                     self.FC6_2_4, self.FC6_2_5, self.FC6_2_6)
    ...
    self.FC_8 = torch.nn.Linear(
        in_features=2000,  # 2048
    ...
    self.branch_2_feats = torch.nn.Sequential(self.shared_layers, self.conv4_2,
                                              self.conv5_2)
...
def train_mc(freeze_feature, resume=None):
  ...
  print('=> Best accuracy at epoch %d, test accuaray %f' %
        (best_epoch, best_acc))
...
def train(resume):
  ...
  for epoch_i in range(30):
    ...
    for batch_i, (data, label) in enumerate(train_loader):  # 遍历每一个batch
      ...
      if iter_count % 10 == 0:
        print('=> epoch {} iter {:>4d}/{:>4d}'
              ', total_iter {:>6d} '
              '| loss {:>5.3f} | accuracy {:>.3%}'.format(
                  epoch_i + 1, batch_i, len(train_loader), iter_count,
                  loss.item(), batch_acc))
    ...
    print('=> epoch {} | average loss: {:.3f} | average accuracy: {:>.3%}'.
          format(epoch_i + 1,
                 float(sum(epoch_loss)) / float(len(epoch_loss)), train_acc))
    ...
    print('\t%d \t%4.3f \t\t%4.2f%% \t\t%4.2f%%' %
          (epoch_i + 1, sum(epoch_loss) / len(epoch_loss), train_acc * 100.0,
           test_acc))
  print('=> Best accuracy at epoch %d, test accuaray %f' %
        (best_epoch, best_acc))
